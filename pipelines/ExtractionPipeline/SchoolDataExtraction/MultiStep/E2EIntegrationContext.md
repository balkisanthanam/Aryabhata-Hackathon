...
4. (To be done after stage 1)
    a) Once stage one is completed, we get Class, Subject (both already known) and ChapterNumber from the just extracted JSON. With this connect to the PostgreSQL (configured via AZURE_PG_HOST environment variable) and get the ChapterId.
...
5. (To be done after stage 2)
    ...
    Loop through the corresponding "questions" (for that Exercise in the json). For each question snippet ({"question_id":"2.2","question_text":"...","page_number":12,"visual_required":true,"visual_data":{"type":"GRAPH","description":"...","box_2d":[340,405,505,650],"visual_source":"current_page","cropped_image_path":"cropped_images\\keph102\\q2_2_fig.png"},"figure_references":["Fig2.9"]})
        i) If "visual_required" is true, then get the "cropped_image_path". This will point to the corresponding image for that question. Upload into the Azure storage (configured via AZURE_STORAGE_ACCOUNT environment variable), container onlineresources. Get the resource's URL
